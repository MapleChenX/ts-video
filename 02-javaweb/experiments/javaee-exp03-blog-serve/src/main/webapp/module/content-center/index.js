    ...
    for (let i = 0; i < res.data.length; i++) {
      ...
      let template = `
        <div class="post-item mar-b-10 pad-all-10 border-b-dotted">
          <div class="post-title mar-b-10 size-18"><a href="./view/post?id=${res.data[i].id}">${res.data[i].title}</a></div>
          <div class="post-desc size-14">${res.data[i].desc}</div>
          <div class="view-post size-13 mar-tb-10"><a href="./view/post?id=${res.data[i].id}">阅读全文>></a></div>
